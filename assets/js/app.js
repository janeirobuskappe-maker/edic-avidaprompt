...
import { renderPlayground, bindPlayground } from "./playground.js";
window.openMindScript = openMindScript;

/* ── Échappement HTML (anti-XSS pour contenu importé/utilisateur) ── */
function esc(s) {
...
function navigate(view, zone = null) {
  ...
  else if (view === "dev")  main.innerHTML = renderDevZone();
  ...
  else if (view === "playground") main.innerHTML = renderPlayground();

  if (view === "editor") bindEditor();
  if (view === "sec")    bindSec();
  if (view === "playground") bindPlayground();
...
/* ═══ ZONE DEV (scripts + accès Playground) ═══ */
function renderDevZone() {
  const scripts = loadScripts().filter(s => s.zone === "dev");
  return `
    <div class="topbar">
      <h1 class="topbar__title">⚙️ Développement</h1>
      <div class="topbar__actions">
        <a href="#playground" class="btn btn--ghost">🎮 Playground</a>
        <button class="btn btn--primary" id="zone-new" data-zone="dev">+ ${t("btn.new")}</button>
      </div>
    </div>
...
function renderZone(zone) {
  ...
  return `
    <div class="topbar">
      ...
      <button class="btn btn--primary" id="zone-new" data-zone="${zone}">+ ${t("btn.new")}</button>
    </div>
    <div class="grid-cards">
      ${scripts.length ? scripts.map(renderScriptCard).join("")
        : `<p class="muted" data-i18n="empty.scripts">Rien ici. Encore. Écris le premier.</p>`}
...
function renderScriptCard(s) {
  return `
    <article class="script-card" data-zone="${esc(s.zone)}" data-id="${esc(s.id)}">
      <header class="script-card__head">
        <span class="badge" data-zone="${esc(s.zone)}">${esc(String(s.zone).toUpperCase())}</span>
        <span class="script-card__score">${esc(s.score)}/10</span>
      </header>
      <h3 class="script-card__title">${esc(s.title)}</h3>
      <p class="script-card__desc">${esc(s.desc)}</p>
      <footer class="flex between center">
        <span class="script-card__meta">${esc(s.date)}</span>
...
/* ═══ BOUTON "+ Nouveau script" DANS UNE ZONE ═══ */
function bindZoneNewButton() {
  document.getElementById("zone-new")?.addEventListener("click", e => {
...
/* ═══ MENU + EXPORT/IMPORT ═══
   Le menu (et export/import) est recréé à chaque fois que la home
   se ré-affiche, donc on utilise une délégation d'événements sur
   #main-content, branchée une seule fois, plutôt que de rebrancher
   des listeners sur des éléments qui disparaissent au re-render. */
function bindMenu() {
  const main = document.getElementById("main-content");
  if (!main) return;
  ...
  main.addEventListener("click", e => {
    if (e.target.closest("[data-menu-toggle]")) {
      e.stopPropagation();
      document.getElementById("menu-main")?.classList.toggle("open");
      return;
    }
    if (e.target.id === "menu-export") {
      exportJSON();
  ...
  main.addEventListener("change", async e => {
    if (e.target.id !== "menu-import") return;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function handleHash() {
    ...
    else if (h === "dev") navigate("dev");
    else if (h === "mind") navigate("mind");
    else if (h === "playground") navigate("playground");
    else navigate("home");
  }
